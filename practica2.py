...
# Ancho del mapa mostrado (la altura se ajusta sola por aspecto)
...
def dms_to_decimal(text: str) -> float:
    """Convierte DMS (p.ej. 40°23'33.11"N) a grados decimales."""
    match = _DMS_RE.match(text)
    if not match:
        raise ValueError(f"Formato DMS inválido: {text!r}")
...
def map_config_insia(display_width: int) -> MapConfig:
    # INSIA: esquinas en lat/lon (DMS) obtenidas con pins y conversión a UTM.
    lat_tl = dms_to_decimal('40°23\'16.8663"N')
    lon_tl = dms_to_decimal('3°38\'4.5268"W')
    lat_br = dms_to_decimal('40°23\'12.3787"N')
    lon_br = dms_to_decimal('3°37\'56.1685"W')

    e_tl, n_tl, _ = geo_to_utm(lat_tl, lon_tl, "WGS84", DEFAULT_ZONE)
    e_br, n_br, _ = geo_to_utm(lat_br, lon_br, "WGS84", DEFAULT_ZONE)
...
def map_config_campus_sur(display_width: int) -> MapConfig:
    # Campus Sur: esquinas tomadas en Google Earth (DMS)
    # TL: 40°23'33.11"N, 3°38'9.48"W (arriba-izquierda)
    # BR: 40°23'9.27"N,  3°37'20.90"W (abajo-derecha)
    lat_tl = dms_to_decimal('40°23\'33.11"N')
    lon_tl = dms_to_decimal('3°38\'9.48"W')
    lat_br = dms_to_decimal('40°23\'9.27"N')
    lon_br = dms_to_decimal('3°37\'20.90"W')
...
# UTM -> píxel (según la georreferenciación del mapa)
...
class GPSMapApp:
    def __init__(self, root: tk.Tk, cfg: MapConfig):
        ...
        # Cargamos y escalamos la imagen para mostrarla
        try:
            self.base_image_pil = Image.open(cfg.image_path)
        except Exception as exc:
            raise RuntimeError(
                f"No se pudo abrir la imagen del mapa: {cfg.image_path}. "
                f"Copia el JPG/PNG al mismo directorio del script o ajusta la ruta. Detalle: {exc}"
            ) from exc
        ...
        # Canvas con el tamaño en pantalla
        ...
        # Guardamos el último punto para poder dibujar una traza
        self.last_x_disp = None
        self.last_y_disp = None

        # Lado derecho: datos del fix
    ...
    def update_position(self, gga: GGAData) -> None:
        ...
        if inside_original_image(self.cfg, x_orig, y_orig):
            ...
            # Traza simple: une puntos consecutivos
...
# Lectura serie (en un hilo para no bloquear la GUI)
...
def main() -> None:
    ...
    parser.add_argument(
        "--map",
        choices=["insia", "campus"],
        default="insia",
        help="Mapa a mostrar: insia o campus",
    )
    ...
    cfg = map_config_insia(args.display_width) if args.map == "insia" else map_config_campus_sur(args.display_width)
